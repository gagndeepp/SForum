...
<body id="_5" style="background-image: url(./img/image1.jpg)">
	...
	<div>
		...
		<ul id="nav-bar">
			...
			<span class="badge badge-primary " style="font-size: 25px;">${emp.getEmpName()}</span>
	...
	<c:set var="post" value="${selectedpost}" />

	<div class="card m-3 shadow-lg text-white bg-primary">

		<div class="card-body">

			<h4 class="card-title ">Question No. ${ post.getPostId()}</h4>

			<h6 class="card-subtitle mb-2">Asked By:

				${post.getEmployeeP().getEmpAlias()}</h6>

			<h6 class="card-subtitle mb-2 ">Posted on :

				${post.getPostDate().toString()}</h6>

			<p class="card-text bg-secondary font-weight-bold">Post

				Description : ${post.getPostDesc()}</p>
			...
			<form:form action="addPostAnswer" modelAttribute="answer">
				...
				<div class="card ml-5 mr-5  shadow-lg text-white bg-info">
					...
					<div class="card-body">
						...
						<form:input type="hidden" name="currentPostId" value=" ${post}  "
							path="postA" />
	...
	<c:forEach items="${answers}" var="currentAnswer">
		<div class="card ml-4 mr-4 mb-2 mu-2 shadow-lg">

			<div class="card-body">

				<h4 class="card-title">Answer No.${currentAnswer.getAnswerId()}</h4>

				<h6 class="card-subtitle mb-2 text-muted">Answered By

					${currentAnswer.getEmployeeA().getEmpAlias()}</h6>
		...
		<form:form action="addPostAnswer" modelAttribute="answer">
			...
			<div class="card m-5 shadow-lg text-white bg-info">
				...
				<div class="card-body">
					...
					<form:input type="hidden" value="${selectedpost.postId}" path="postA" />
